...
//Define the CC1101 SPI pins in globals.h - Set up new SPI GPIOs for CC1101:
#define VSPI_SCK   13    // GPIO SPI Clock  
#define VSPI_MISO  15    // GPIO Data Input
#define VSPI_MOSI  12    // GPIO Data Output
#define VSPI_CS    25    // Chip Select - Active LOW

// ESP32 Hardware pin configuration 

#ifdef ESP32
   #define PROC_TYPE "ESP32" //>>>>>>>>>>>>>>>>>>>>GDOx0=14 & GDO2=26 Works!!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
   #define USE_GPIO_Dx 0  //GPIO-0 is reserved
   const int16_t RXPin = 14;      //CC1101 serial output to uP - (also known as GDO2) must be an ext interrupt pin
   const int16_t CDPin = 26;      //CC1101 carrier detect output (also known as GDO0) to uP >>if wired<<, define 'USE_HW_CD' above, 
                                  //       otherwise CD pin status is received over SPI
   const int16_t CC1101_CS = 25;  //CC1101 Chip Select Input pin - same as VSPI_CS (Chip Select) above
...
//can be turned on/off using the #define USE_TEST_TIMINGS at the top of this include file
      ...
      {
      52,56,44,56,44,52,48,52,100,196,96,100,104,96,56,40,60,40,60,44,52,44,108,44,52,92,112,88,56,44,
      52,48,104,96,96,100,104,44,56,92,104,48,52,96,104,44,56,92,56,44,104,44,56,92,104,48,52,44,60,40,
      60,92,104,92,48,52,100,52,44,52,56,100,92,100,52,52,48,48,104,48,48,100,92,56,52,48,52,44,52,48,
      52,96,100,100,48,48,52,56,48,44,100,56,48,44,56,92,48,52,56,44,96,52,56,44,56,44,52
      };
